...
function matchesRepo(event: TaskEvent, query: string): boolean {
  return event.repo === query || event.repoName === query || event.repo?.includes(query) === true;
}
...
const args = rawArgs[0]?.endsWith("task-events-state.ts") ? rawArgs.slice(1) : rawArgs;
let taskId = "";
let repo = "";
let limit = 20;
...
const summaries = Array.from(byTask.entries())
  ...
  .sort((a, b) => (a.latestEvent?.timestamp ?? "").localeCompare(b.latestEvent?.timestamp ?? ""))
  .slice(-limit);
