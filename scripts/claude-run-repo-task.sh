...
emit_task_event() {
  ...
  if ! "${cmd[@]}" >/dev/null; then
    echo "==> [events] WARNING: failed to emit event type=$event_type status=$status (non-fatal)" >&2
  fi
}
...
notify_openclaw() {
  ...
  if openclaw agent --agent main --message "$msg" >/dev/null 2>&1; then
    echo "==> [notify] OpenClaw wake sent ($state)"
  else
    echo "==> [notify] WARNING: OpenClaw notify FAILED (exit $?) — state '$state' was NOT delivered; check openclaw agent health" >&2
  fi
}
