...
# Send an immediate wake notification to the active OpenClaw session.
# Best-effort: never fails the run if openclaw is unavailable or the call errors.
notify_openclaw() {
  local state="$1"
  local pr_url="${2:-}"
  ...
  local msg="Claude run done: $REPO_NAME / $BRANCH — $state"
  if [[ -n "$pr_url" ]]; then
    msg="$msg — $pr_url"
  fi
...
# Track terminal state for the single completion notification (most specific wins).
