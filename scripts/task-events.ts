...
export async function appendTaskEvent(
  event: Omit<TaskEvent, "eventId" | "timestamp"> & Partial<Pick<TaskEvent, "eventId" | "timestamp">>,
...
export async function readTaskEvents(eventsPath = DEFAULT_EVENTS_PATH): Promise<TaskEvent[]> {
  await ensureFile(eventsPath);
...
async function main(): Promise<void> {
  ...
  const args = rawArgs[0]?.endsWith("task-events.ts") ? rawArgs.slice(1) : rawArgs;
  ...
  if (command === "emit-fields") {
    ...
    const payload = flags.payloadFile
      ? (JSON.parse(await readFile(flags.payloadFile, "utf8")) as Record<string, unknown>)
      : flags.payloadJson
        ? (JSON.parse(flags.payloadJson) as Record<string, unknown>)
        : {};
    const event: Omit<TaskEvent, "eventId" | "timestamp"> & Partial<Pick<TaskEvent, "eventId" | "timestamp">> = {
