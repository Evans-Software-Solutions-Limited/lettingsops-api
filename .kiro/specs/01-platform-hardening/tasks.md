...
- [x] **I1.** Add `agentAgencyMap` table (`agent_id PRIMARY KEY` text, `agency_id NOT NULL` uuid + FK to agencies, `notes` text, audit columns). Migration `0003_agent_agency_map.sql` + Drizzle schema + snapshot landed. _Seeding intentionally deferred to I-PR-C (when the webhook actually starts reading the lookup) so this PR stays purely additive — a seed in the migration would be non-idempotent on re-deploy. Operators will run a one-shot `INSERT ... ON CONFLICT DO NOTHING` per known ElevenLabs agent after I-PR-C deploys to staging._
- [x] **I2.** `AgentAgencyRepository.findAgencyForAgent(agentId)` at `microservices/core/src/application/auth/agentAgencyRepository.ts`. Returns `string | null` — null is treated as authentication failure by callers. Also exposes `upsert(...)` for the seed/admin path. Tests cover hit, miss, malformed row (undefined `agencyId` collapses to null), `.limit(1)` query shape, upsert success, upsert no-row error, optional `notes`. _Repo lives in `application/auth/` alongside `apiKeyRepository.ts` since both are identity-resolution tables that don't extend `TenantScopedRepository`._
- [x] **I3.** `elevenLabsWebhookService.ts` resolves `agencyId` via `new AgentAgencyRepository(db).findAgencyForAgent(payload.agentId)` before constructing the `LeadRepository`. Unknown agentId throws `HttpError(401, "Unknown agent — no agency mapping for this ElevenLabs agent")`; the handler's new `.onError` block maps it to a 401 response so ElevenLabs stops retrying (the `ElevenLabsWebhookFailures` alarm from Block G catches sustained misses operationally). `ANY_AGENCY` import dropped from the file. _Landed alongside I4 in the I-PR-B+C combined PR._
- [x] **I4.** Shared `resolveAgencyFromInboundEmail(recipientEmail, db?)` extracted to `application/ingestion/email/agencyResolver.ts`. Both ingestion paths use it: `emailProcessor.ts` (Lambda) calls it then short-circuits on null; the HTTP wrapper `EmailIngestionService.processEmail` calls it and throws `HttpError(401, "No agency owns the recipient address")` on null. **Contract change** for `POST /webhooks/email`: body now requires `to` (recipient address). `ANY_AGENCY` import dropped from `emailIngestionService.ts`; `processEmail` signature tightened from `AgencyScope` to plain `string`. `emailIngestionHandler` gains the same `.onError` block as the seven business handlers.
- [x] **I5.** `ANY_AGENCY`, `isAny()`, the bypass paths in `scopeWhere`/`writeAgencyId`, and the `AgencyScope` alias deleted from `tenantScopedRepository.ts`. All four repos (`leadRepository`, `viewingRepository`, `qualificationRepository`, `conversationRepository`) now declare `agencyId: string` on their constructors; the dead `if (agencyId === undefined) throw` defensive branch in `ConversationRepository.create` is also gone (the signature blocks the path at compile time). _Column DEFAULT (`LEGACY_AGENCY_ID`) deliberately kept for now — drop it in a follow-up migration once one preprod release has shipped with the lookups in place, per the original plan. Landed in I-PR-D._
- [x] **I6.** Sentinel-specific suites in `tenantIsolation.test.ts` and `tenantScopedRepository.test.ts` removed (4 cases total: reads + writes scoped to sentinel, no-UUID-in-WHERE, ConversationRepository sentinel-refusal). The cross-tenant isolation matrix preserved. Repo-level tests that previously constructed with `ANY_AGENCY` now use a fixed `"agency-test-1"` fixture; `emailProcessor.integration.test.ts` likewise. Helpers that only served the sentinel path (`whereDoesNotScopeTo`, `valuesOmitAgencyId`) removed. Comment-level references to `ANY_AGENCY` are retained throughout as documentation of the retirement. _Landed in I-PR-D._

> **Block I done.** No production code path uses the `ANY_AGENCY` sentinel. The `LEGACY_AGENCY_ID` column DEFAULT is still in place as a transitional safety belt — drop it in a follow-up migration after one preprod release has confirmed the resolver paths are healthy. The webhook seed (one `agent_agency_map` row per ElevenLabs agent) is still a pre-deploy step.
